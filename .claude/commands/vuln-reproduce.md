...
| 优先级 | 模式 | 适用场景 | 使用条件 |
|--------|------|---------|---------|
| **1st** | L1 Playwright | **默认首选** | 能在浏览器中验证的漏洞 |
| **2nd** | L2 API 接口 | L1 无法完成时 | 纯后端 API、无页面效果 |
| **3rd** | L3 Mock 代码 | **最后手段** | L1/L2 都失败、需内部状态 |
...
**选择 L1**：复现步骤涉及页面操作、需要观察渲染结果、需要 JS 执行环境

**选择 L2**：漏洞报告提供了 API 端点、curl 能验证、不需要页面渲染

**选择 L3**：L1/L2 失败后、需要特定内部状态、复杂竞态条件

### 关键原则

1. **不要预设**：不要看到漏洞类型就直接选模式，读完报告再判断
2. **先简后繁**：能用简单方式验证的，不要用复杂方式
3. **失败降级**：当前模式失败时，再考虑下一级模式
